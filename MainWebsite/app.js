...
app.get("/registration", (req, res) => {
  res.status(200).render('RegistrationPage/registration.pug');
})
...
app.get("/login", (req, res) => {
  res.status(200).render('LoginPage/login.pug');
});
  ...
  con.connect(function (err) {
    ...
    con.query(sql, function (err, result) {
      ...
      if (res_db == pass) {
        ...
        res.status(200).render('LoginPage/successlogin.pug', mes);

      }
      else {

        const mes = { 'message': 'Failed to Login..Check your email or password ' };
        res.status(200).render('LoginPage/failedlogin.pug', mes);
